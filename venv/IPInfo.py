...
#
#         if json == True:
#             return '{"ip":"%s", "cArea":"%s", "aArea":"%s", "time":"%s", "array":%s}' % (
#             ips, c, a, str(time.time() - ts), city_analyst(c + ":" + a, json=True))
#         else:
#             return '<pre>%s %s %s<br><br>运行时间：%f 秒<br><br>%s</pre>' % (
#             ips, c, a, time.time() - ts, city_analyst(c + ":" + a))
#     else:
